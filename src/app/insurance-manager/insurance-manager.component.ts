  ...
  rejectSubscription(id: number): void {
    this.insuranceService.rejectSubscription(id).subscribe({
      next: () => {
        ...
        this.showSuccessMessage('Subscription rejected successfully.');
      },
      error: (err: any) => {
        // Log the error for debugging
        console.error('Error rejecting subscription:', err);
        // Display a more specific error message
        const errorMsg = err.error?.message || err.message || 'Failed to reject subscription. Please check your connection or try again later.';
